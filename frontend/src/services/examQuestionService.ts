...
export interface ExamQuestionData {
  ...
  language?: 'th' | 'en';
  sourceType?: 'text' | 'pdf';
  questionType?: 'text' | 'writing' | 'choice' | 'pdf';
  options?: string[];
...
function parsePdfPageNumbers(prompt: string): number[] {
...
function formatPageSliceName(questionNumber: string, pages: number[], lineIndex: number): string {
  if (pages.length === 1) {
...
function inferQuestionType(prompt: string): { questionType: ExamQuestionData['questionType']; options?: string[] } {
...
export function getAnswerMethod(question: ExamQuestionData): string {
